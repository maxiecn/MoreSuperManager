...
namespace MoreSuperManager.DAL
{
    public class ModuleDAL
    {
        ...
        public List<DBModuleFullModel> Page(string channelCode, string searchKey, int pageIndex, int pageSize, ref int totalCount, ref int pageCount)
        {
            ...
            parameterList.Add("@FieldSql", "IdentityID, ModuleCode, ModuleName, ActionList, ChannelCode, (select ChannelName from T_Channel with(nolock) where T_Channel.ChannelCode=T.ChannelCode) as ChannelName");
            parameterList.Add("@Field", "IdentityID, ModuleCode, ModuleName, ActionList, ChannelCode");
